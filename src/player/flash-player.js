	...
	var messageNameMap = {
		...
		addFlashEvents = function(player) {
			var map = MTVNPlayer.Player.flashEventMap,
				...
				fullScreenChange = MTVNPlayer.Events.FULL_SCREEN_CHANGE,
			...
			map[id + fullScreenChange] = function(isFullScreen) {
				if (player.isFullScreen !== isFullScreen) {
					player.isFullScreen = isFullScreen;
					player.trigger(fullScreenChange, isFullScreen);
				}
			};
			element.addEventListener('FULL_SCREEN_CHANGE', mapString + fullScreenChange);
